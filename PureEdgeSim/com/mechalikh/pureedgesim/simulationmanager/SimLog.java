...
public class SimLog {
	...
	private double totalSuccExecTime;
	private double totalSuccWaitingTime;
	private double totalSuccNetworkTime;
	private double totalLatencyPenalty = 0.0;
	...
	private int numHighSecInUE = 0;
	private int numHighSecInNearestEdge = 0;
	private int numHighSecInOther = 0;

	public void setTaskFiledDueToEnergy(int taskFiledDueToEnergy) {
		this.taskFiledDueToEnergy = taskFiledDueToEnergy;
	}

	private int taskFiledDueToEnergy = 0;
	...
	private void printDCwiseCounter(){
		...
		if(simulationManager.getScenario().getStringOrchAlgorithm().equals("GS_AGRI")) {
			for (DataCenter dc : simulationManager.getDataCentersManager().getEdgeDatacenterList()) {
				System.out.println("DC= " + dc.getName() + " Value: " + dc.getNumTasks());
				index++;
			}
		} else {
	...
	private void printApplicationSpecificData(){
		Map<Integer, Map<Integer, TaskNode>> tasksMap = DataProcessor.tasksMap;
		int numAppFailed = 0;
		double totalSuccAppExecutionTime = 0.0;
		for(Map.Entry<Integer, Job> entry : DataProcessor.scheduledJob.entrySet()){
			Integer id = entry.getKey();
			if(tasksMap.containsKey(id)){
				Map<Integer, TaskNode> job = tasksMap.get(id);
				...
				if(isSucceed){
					...
					totalSuccAppExecutionTime += duration;
				}
			} else{
				System.out.println("The key should be present...you need to check.");
			}
		}

		int succApps = DataProcessor.scheduledJob.size() - numAppFailed;
		System.out.println("Total successful App execution time: " + totalSuccAppExecutionTime +" seconds");
		System.out.println("Average successful App execution time: " + totalSuccAppExecutionTime/(double) succApps +" seconds");
		System.out.println("No of App succeed/failure: " + (succApps)+ "/"+numAppFailed);
	}

	private void printGeneric(){
		double mistEnConsumption = 0;
		printApplicationSpecificData();
		double totalTaskCompletionTime = totalExecutionTime + totalWaitingTime + totalNetworkTime;
		...
		System.out.println("Total task waiting time: " + decimalFormat.format(totalWaitingTime) + " seconds");
		System.out.println("Average task waiting time: " + decimalFormat.format(totalWaitingTime / executedTasksCount) + " seconds");
		System.out.println("Total task network time: " + decimalFormat.format(totalNetworkTime) + " seconds");
		System.out.println("Average task network time: " + decimalFormat.format(totalNetworkTime / executedTasksCount) + " seconds");
		System.out.println("Total task completion time: " + decimalFormat.format(totalTaskCompletionTime) + " seconds");
		System.out.println("Average task completion time: " + decimalFormat.format(totalTaskCompletionTime/executedTasksCount) + " seconds");
		...
		System.out.println("Task Failed executed: " +  decimalFormat.format((double)  tasksFailed * 100 / tasksSent)
				+ " % (" + tasksFailed + " among " + tasksSent + " sent tasks)");
		//System.out.println("Task Executed on the cloud: "+(tasksExecutedOnCloud - tasksFailedCloud));
		...
		System.out.println("Number of edge device: "+simulationManager.getDataCentersManager().getEdgeDevicesList().size());
		...
		System.out.println("Total UE energy consumption: "+ decimalFormat.format(mistEnConsumption) + " Wh");
		System.out.println("Average UE energy consumption: "+ decimalFormat.format((double) mistEnConsumption/simulationManager.getDataCentersManager().getEdgeDevicesList().size()) + " Wh");
		System.out.println("Total UE devices: "+decimalFormat.format(simulationManager.getDataCentersManager().getEdgeDevicesList().size()));
		System.out.println("Total number of critical tasks failed: "+numCriticalTaskFailed);
		System.out.println("Total number of tasks failed due to Energy: "+taskFiledDueToEnergy);

		Integer totalSuccTasks = tasksSent - tasksFailed;
		double totalSuccTime = this.totalSuccExecTime + this.totalSuccWaitingTime+this.totalSuccNetworkTime;
		System.out.println("Total task sent: "+tasksSent);
		System.out.println("Total task failed: "+tasksFailed);
		System.out.println("Total task succeed: "+totalSuccTasks);
		System.out.println("Total execution time for successful tasks: "+decimalFormat.format(this.totalSuccExecTime)+ " seconds");
		System.out.println("Total waiting time for successful tasks: "+decimalFormat.format(this.totalSuccWaitingTime)+ " seconds");
		System.out.println("Total network time for successful tasks: "+decimalFormat.format(this.totalSuccNetworkTime)+ " seconds");
		...
		System.out.println("High security tasks executed in UE/nearestEdge/Other " +numHighSecInUE+"/"+numHighSecInNearestEdge+"/"+numHighSecInOther);
		System.out.println("Task succesfull execution level (UE/EDGE/CLOUD): " + (tasksExecutedOnMist - tasksFailedMist) + "/"
				+ (tasksExecutedOnEdge - tasksFailedEdge) + "/" + (tasksExecutedOnCloud - tasksFailedCloud));
		System.out.println("Task sent level (UE/EDGE/CLOUD): " + tasksExecutedOnMist + "/"
				+ tasksExecutedOnEdge + "/" + tasksExecutedOnCloud);
	}
	...
	public void incrementTasksNotSent() {
		this.tasksNotSent++;
	}

	public void addLatencyPenalty(double penalty){
		totalLatencyPenalty += penalty;
	}
	...
	public void getTasksExecutionInfos(Task task) {
		...
		if(task.getStatus() == Task.Status.SUCCESS){
			this.totalSuccExecTime += task.getActualCpuTime();
			this.totalSuccWaitingTime += task.getWatingTime();
			this.totalSuccNetworkTime += task.getActualNetworkTime();
			//double exec = task.getActualCpuTime()+task.getWatingTime()+task.getActualNetworkTime();
			//System.out.println("TaskId: "+task.getId()+" task Length: "+task.getLength()+" Server MIPS: "+task.getComputingNode().getMipsPerCore()+ " Execution Time: "+exec);
		}
	}
	...
	public void incrNumHighSecInUE(){
		numHighSecInUE++;
	}

	public void incrNumHighSecInNearestEdge(){
		numHighSecInNearestEdge++;
	}

	public void inrNumHighSecInOther(){
		numHighSecInOther++;
	}
}
