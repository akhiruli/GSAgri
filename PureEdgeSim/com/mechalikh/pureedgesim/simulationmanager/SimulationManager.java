...
public class SimulationManager extends SimulationManagerAbstract {
	...
	private void sendTaskToOrchestrator(Task task) {
		if (taskFailed(task, 0)) {
			//System.out.println("sendTaskToOrchestrator- Task: "+task.getId() + " app_id: " + task.getApplicationID());
			//return;
		}

		//System.out.println("sendTaskToOrchestrator Task: "+task.getId() + " app_id: " + task.getApplicationID() + " : " + task.getComputingNode().getType());
		//taskFailed(task, 0);
	...
	public boolean taskFailed(Task task, int phase) {
		...
		if (task.getEdgeDevice().isDead()) {
			if(task.getComputingNode() == ComputingNode.NULL){
				task.setComputingNode(task.getEdgeDevice());
			}
