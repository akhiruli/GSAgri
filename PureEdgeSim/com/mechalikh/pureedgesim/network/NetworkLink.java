...
public class NetworkLink extends SimEntity {
	...
	private boolean realNetwork = false;
	...
	protected double getBandwidth(double remainingTasksCount) {
		if(realNetwork){
			double realBw = NetworkDataset.getInstance().getData(simulationManager.getSimulation().clock());
			//System.out.println("AKHIRUL: "+realBw+" : "+(realBw / (remainingTasksCount > 0 ? remainingTasksCount : 1)));
			return (realBw / (remainingTasksCount > 0 ? remainingTasksCount : 1));
		}else {
			//System.out.println("AKHIRUL: "+bandwidth+" : "+(bandwidth / (remainingTasksCount > 0 ? remainingTasksCount : 1)));
